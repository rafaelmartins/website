...
type contentProvider interface {
	...
	Render(f string, baseurl string, withToc *bool) (*frontmatter.FrontMatter, string, string, error)
...
func Render(f string, baseurl string, withToc *bool) (*frontmatter.FrontMatter, string, string, error) {
	p := getProvider(f)
	if p == nil {
		return nil, "", "", fmt.Errorf("content: no provider found: %s", f)
	}
	return p.Render(f, baseurl, withToc)
}
